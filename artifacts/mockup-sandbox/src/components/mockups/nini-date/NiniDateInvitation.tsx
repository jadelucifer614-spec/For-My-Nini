...
function Ambient() {
  ...
  return (
    <div aria-hidden="true" className="pointer-events-none fixed inset-0 overflow-hidden">
      ...
      <div className="nini-heart-field">
        {[0, 1, 2, 3, 4].map((i) => (
          <motion.span
            key={`heart-${i}`}
            className="nini-floating-heart"
            style={{ left: `${12 + i * 19}%`, top: `${18 + (i % 3) * 25}%`, animationDelay: `${i * 1.3}s` }}
          >
            <Heart size={i % 2 === 0 ? 14 : 10} fill="currentColor" />
          </motion.span>
        ))}
      </div>
      <div className="nini-sparkle-field">
        {[0, 1, 2, 3, 4, 5].map((i) => (
          <span key={`sparkle-${i}`} className="nini-floating-sparkle" style={{ left: `${8 + i * 17}%`, top: `${12 + (i % 4) * 21}%`, animationDelay: `${i * .8}s` }}>
    ...
    <main className="nini-shell">
      ...
      <AnimatePresence mode="wait">
        <motion.section key={screen} className="nini-stage" initial={{ opacity: 0, y: reduceMotion ? 0 : 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: reduceMotion ? 0 : -18 }} transition={transition}>
          {screen === 1 && <div className="nini-content nini-letter"><div className="nini-letter-mark"><Sparkles size={14} /> for one Teddy only</div><p className="nini-kicker">open when you have a minute</p><h1 className="nini-title">Hi Nini <em>♡</em></h1><p className="nini-copy">I have a very important question to ask you...</p><div className="nini-actions"><button className="nini-button nini-button-rose" onClick={next}>I'M LISTENING <ArrowRight size={15} style={{ verticalAlign: "middle", marginLeft: 8 }} /></button></div><p className="nini-note">from Fifi</p></div>}
          {screen === 2 && <div className="nini-content nini-glass nini-invitation"><span className="nini-badge"><Heart size={13} fill="#d86683" color="#d86683" /> a very official invitation</span><h2 className="nini-title" style={{ fontSize: "clamp(34px,8vw,66px)" }}>Teddy would like to take Nini on a date.</h2><p className="nini-copy">Unfortunately, we're still stuck behind our screens...</p><p className="nini-copy" style={{ marginTop: 10 }}>So I made our own little online date instead.</p><h3 className="nini-question" style={{ marginTop: 25 }}>Will you join me?</h3><Hearts active={burst} /><div className="nini-actions"><button className="nini-button nini-button-rose" onClick={() => { setBurst(true); window.setTimeout(next, 800); }}>{maybeMode ? "YES ♡" : "YES, OBVIOUSLY ♡"}</button><button className="nini-button nini-button-soft" onClick={() => { if (maybeMode) { setBurst(true); window.setTimeout(next, 800); } else setMaybeMode(true); }}>{maybeMode ? "YES ♡" : "maybe..."}</button></div>{maybeMode && <p className="nini-note nini-playful-note" style={{ color: "#d86683" }}>Nini. 😐</p>}</div>}
